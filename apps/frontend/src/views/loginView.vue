...
const login = async () => {
  ...
  try {
    ...
    if (userError || !userData) {
      toast.error("Usuario no autorizado o inactivo.");
      // Cerrar sesión si hay problema
      await supabase.auth.signOut();
      return;
    }

    // 3. Guardar información en localStorage
...
onMounted(async () => {
  ...
  if (session?.user) {
    // Verificar que el usuario siga siendo válido en nuestra tabla
    ...
    if (userData) {
      // Usuario válido, actualizar localStorage y redirigir
      localStorage.setItem("userRole", userData.rol);
      localStorage.setItem("userName", userData.nombre);
